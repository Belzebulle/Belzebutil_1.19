...
import fr.krepe.belzebutil.screen.renderer.EnergyInfoArea;
import fr.krepe.belzebutil.util.MouseUtil;
...
public class LeadStationScreen extends AbstractContainerScreen<LeadStationMenu> {
    private static final ResourceLocation TEXTURE =
            new ResourceLocation(fr.krepe.belzebutil.Belzebutil.MOD_ID, "textures/gui/lead_station_gui.png");
    ...
    protected void init() {
        ...
        energyInfoArea = new EnergyInfoArea(((width - imageWidth) / 2) +  156,
                ((height - imageHeight) / 2) + 7,9,71,menu.blockEntity.energyStorage);
    }

    public LeadStationScreen(LeadStationMenu p_97741_, Inventory p_97742_, Component p_97743_) {
    ...
    protected void renderBg(PoseStack pPoseStack, float p_97788_, int xMouse, int yMouse) {
        ...
        /*
        if (menu.isCrafting()) {
            this.blit(pPoseStack, x + 54, y + 38 , 176, 0, 14,  menu.getScaledProgress());
        }
        */
    ...
    protected void renderLabels(PoseStack poseStack, int xMouse, int yMouse) {
        ...
        if(isMouseAboveArea(xMouse, yMouse, x, y , 156, 7 , 9, 70)){
